...
public class SendToSparkServiceSocket implements SatelliteServiceSocket{
    ...
    private void convertAndSend(final Set<Path> images,final String manifest){
        ...
        IntStream.range(1,y).forEach(rowId->{
            IntStream.range(1,x).forEach(colId->{
                ...
                images.forEach(image->{
                    ...
                    if (postfixSearchArray.length>0){
                        try {
                            ...
                            ds.GetRasterBand(1).ReadRaster(0,0,3,3,tempArray);
                            final var result = IntStream.range(0,9)
                                    .mapToDouble(i->tempArray[i]).boxed().collect(Collectors.toList());
    ...
    private void send(final DtoSparkImagePart dto){
