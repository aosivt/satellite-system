package org.satellite.system.http
...
import akka.http.scaladsl.server.RouteResult.{route2HandlerFlow, routeToFlow}
...
trait WebServer {
  ...
  implicit lazy val materializer: Materializer = Materializer.matFromSystem(system)
  ...
  def start(host: String, port: Int): Future[Unit] = {
    bindingFuture = Http().newServerAt(host, port).bindFlow(routeToFlow(routes))
